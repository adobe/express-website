...
export default function decorate($block) {
  ...
  let $marqueeBgContent, $frameBgImageContent, $thumbnailsContent, $editorContent, $contentSection;
  $rows.forEach(($row) => {
    ...
    switch ($headerCols.textContent) {
      case 'content':
        if ($a && $a.classList.contains('button')) {
          ...
          if ($h1) {
            $h1.parentNode.classList.add("content-section");
        ...
        if ($a && $a.textContent.startsWith('https://') && $a.href.endsWith('.mp4')) {
          ...
          if ($video) {
            $video.addEventListener('loadstart', () => {
              ...
              const $fullScreenMarqueeSection = $block.closest('.fullscreen-marquee');
