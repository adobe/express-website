...
function renderStillWrapper(template) {
  ...
  const imageThumbnailId = thumbnail.componentId;
  const imageSize = thumbnail.width > thumbnail.height ? thumbnail.width : thumbnail.height;

  const thumbnailImageHref = renditionLinkHref.replace(
    '{&page,size,type,fragment}',
    `&size=${imageSize}&type=image/jpg&fragment=id=${imageThumbnailId}`,
  );
