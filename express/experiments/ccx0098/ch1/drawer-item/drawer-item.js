...
function createIndicators(payload, carousel) {
    const count = parseInt(payload.drawerItemContainer.dataset.itemLoadedCount);
    let eventName = '';
...
function getMissingIcon(iconName) {
    const icons = {
        'scratch': 'blank', 'start-from-scratch': 'blank', 'design-assets': 'design-assets', 'blank': 'blank',
...
function createIcon(name) {
    //alt value would eventually need localization
...
function createCTA(payload,link,isSingleDownloadCTA) {
    ...
    if (payload.hasList) {
        // cta.dataset.ll = cta.dataset.ll + ':iconview';
        ...
        // TODO: see if this needs to be modified to use addFreePlanWidget
        widget.innerHTML = `
...
function decorateAnimationsViewMedia(payload) {
    if (payload.drawerItemContainer.classList.contains('drawer-item-animations-view-container')) {
        payload.drawerItemContainer.querySelectorAll('a').forEach((link) => {
            // payload.$mobileDrawer.querySelectorAll('.drawer-item-container-1 a').forEach((link) => {
...
function updateCTAsOnCarouselScroll(payload) {
    let container,selector,eventName;
    if (payload.isDefaultView) {
    ...
    container = payload.$ctasWrapper?.querySelector(selector);
    if (container) {
...
function updateCTAContainersOnInitialLoad(payload) {
    if (payload.$mobileDrawer && payload.$ctasWrapper) {
        let activeTabButton = payload.$mobileDrawer.parentNode.querySelector('.toggle-carousel-platform > .active');
...
function initAfterDrawerFullyLoaded(payload) {
    const isFullyLoaded = payload.drawerItemContainer.dataset.itemCount === payload.drawerItemContainer.dataset.itemLoadedCount;
    if (isFullyLoaded) {
...
function trackTotalItemsLoaded(payload) {
    payload.drawerItemContainer.dataset.itemLoadedCount = parseInt(payload.drawerItemContainer.dataset.itemLoadedCount) + 1;
}
