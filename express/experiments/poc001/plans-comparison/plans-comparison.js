...
import { createTag, fixIcons, getIconElement, getOffer, fetchPlainBlockFromFragment } from '../../../scripts/scripts.js';
...
function collapseCard($card, payload) {
  ...
  if (window.innerWidth >= 1200) {
    ...
    $card.style.maxHeight = `${$heading.offsetHeight + $subcopy.offsetHeight + payload.cardPadding}px`;
    $card.style.maxWidth = '';
  }
}
  ...
  if (!$card.classList.contains('expanded')) {
    ...
    if (window.innerWidth >= 1200) {
      ...
      $card.style.maxHeight = `${
        $heading.offsetHeight + $subcopy.offsetHeight + $featuresWrapper.offsetHeight + $ctasWrapper.offsetHeight + payload.cardPadding * 2
      }px`;
    }
  }
}
...
export default async function decorate($block) {
  $block.classList.add('experiment-added');
